...
- One-click iOS/iPadOS software-keyboard toggle through Consumer Control Eject
...
3. Select the Mac's current **Computer Name**, which Typephone shows in the pairing instructions.
...
If a previous development build was paired, forget the old keyboard entry first and pair again. Adding the software-keyboard control changes the HID descriptor, so existing development pairings must be recreated once.
...
### Show the phone keyboard

While the phone is connected, choose **Show phone keyboard** in Typephone. The app sends the HID Consumer Control Eject action, which toggles the iOS/iPadOS software keyboard without disconnecting the Mac keyboard.

If the command is unavailable, forget the Typephone keyboard in Bluetooth settings and pair it again. As a fallback, use the phone's own **Show Keyboard** control.
...
- 可通过 Consumer Control Eject 一键切换 iOS/iPadOS 屏幕键盘
...
3. 选择 Mac 当前的**电脑名称**；Typephone 会在配对提示中显示这个名称。
4. 等待 Typephone 显示 HID 输入特征已订阅。
5. 打开备忘录等文本输入界面。
6. 选择镜像或独占模式并开始输入。

如果旧开发版本留下了配对记录，请先在手机上忽略旧键盘再重新配对。由于屏幕键盘控制会改变 HID 描述符，已有开发配对需要重新建立一次。
...
手机连接后，点击 Typephone 中的**显示手机键盘**。应用会发送 HID Consumer Control Eject 指令，在不断开 Mac 键盘的情况下切换 iOS/iPadOS 屏幕键盘。

如果指令不可用，请在手机蓝牙设置中忽略 Typephone 键盘并重新配对。仍无法显示时，可以使用手机系统提供的**显示键盘**入口。
